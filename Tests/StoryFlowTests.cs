...
public class StoryFlowTests
{
    ...
    public void AllChapters_EnsureAllNodesAreReachable()
    {
        foreach (Chapter chapter in DataLayer.Chapters)
        {
            ...
            while (nodesToVisit.Count > 0)
            {
                ...
                else if (currentNode is ActionNode actionNode)
                {
                    foreach (Kriss.Models.Action action in actionNode.Actions)
                    {
                        if (action.ChildId.HasValue)
    ...
    public void AllChapters_ValidateTextContent()
    {
        ...
        foreach (Chapter chapter in DataLayer.Chapters)
        {
            foreach (NodeBase node in chapter.Nodes)
            {
                // Every node should have text or alternative text
                Assert.IsFalse(string.IsNullOrWhiteSpace(node.Text) && string.IsNullOrWhiteSpace(node.AltText),
                    $"Node {node.Id} in chapter {chapter.Id} has no text content");
                ...
                else if (node is ActionNode actionNode)
                {
                    ...
                    foreach (Kriss.Models.Action action in actionNode.Actions)
                    {
                        Assert.IsTrue(action.Verbs.Count > 0,
                            $"Action in node {node.Id}, chapter {chapter.Id} has no verbs");
                    }
                }
            }
        }
